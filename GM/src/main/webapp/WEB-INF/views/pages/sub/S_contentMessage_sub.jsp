	...
	<article>
		...
		<div class="content">
			...
			<c:if test="${ct_dto.message_num == 4 }">
				<input type="button" class="inputButton" value="복원" onclick="window.location='<c:url value="/pages/S_garbageContentRestorePro?pageNum=${pageNum}&num=${ct_dto.num}&message_num=${ct_dto.message_num}" />'">
			...
			<c:if test="${ct_dto.message_num == 1}">
				<input type="button" class="inputButton" value="목록 보기" onclick="window.location='<c:url value="/pages/S_message_inbox?pageNum=${pageNum}&num=${ct_dto.num}&message_num=${ct_dto.message_num}" />'">
			</c:if>
			<c:if test="${ct_dto.message_num == 2}">
				<input type="button" class="inputButton" value="목록 보기" onclick="window.location='<c:url value="/pages/S_sendMessageList?pageNum=${pageNum}&num=${ct_dto.num}&message_num=${ct_dto.message_num}" />'">
			</c:if>
			<c:if test="${ct_dto.message_num == 4}">
				<input type="button" class="inputButton" value="목록 보기" onclick="window.location='<c:url value="/pages/S_garbageList?pageNum=${pageNum}&num=${ct_dto.num}&message_num=${ct_dto.message_num}" />'">
			</c:if>			
			</div>
		</form>
		</div>
