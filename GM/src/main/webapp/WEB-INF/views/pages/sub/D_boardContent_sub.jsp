				...
				<th colspan="4">
					<c:if test="${sessionScope.loginInfo.sys_rank == 1}">
						<c:if test="${sessionScope.loginInfo.name == dto.writer}">
							...
							<input type="button" class="inputButton" value="글 삭제" onclick="window.location='<c:url value="/pages/D_boardDeletePro?boardnum=${dto.boardnum}&pageNum=${pageNum}&num=${num}&del=${dto.del}&ref_level=${dto.ref_level}"/>'">
					</c:if>
