...
if(url.indexOf('/')){
	...
	if(url_split.length > 2){
		if(url_split.indexOf('?')){
			...
			if(url[1] == "/pages/E_orgnaizationMemberList"){
				// alert("됨");
				$(function(){
					$("#E_orgnaizationMemberList").children().css("color", "#d3292c");
					$("#E_orgnaizationMemberList").css("font-weight", "bold");
				});
			} else if(url[1] == "/pages/E_orgMyFavoritMemList"){
				// alert("됨");
				$(function(){
					$("#E_orgMyFavoritMemList").children().css("color", "#d3292c");
					$("#E_orgMyFavoritMemList").css("font-weight", "bold");
				});
			}
		}
	}
